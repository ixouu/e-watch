...
const CartForm = () => {
    ...
    // Function that aim to return an adress depending of user's choices

    const {axiosData, axiosIsLoading, setAxiosUrl, axiosError} = UseAxios();

    const searchAdress = async (e) => {
        const apiUrl = "https://api-adresse.data.gouv.fr/search/?q=";
        const userInput = e.target.value.replaceAll(" ","+").toLowerCase();
        console.log(userInput)
        const urlToFetch = `${apiUrl}+${userInput}`
        UseAxios().setAxiosUrl(urlToFetch);
		let foundData = await UseAxios().axiosData.data.features;
		if (foundData) return console.log(foundData);
	};
